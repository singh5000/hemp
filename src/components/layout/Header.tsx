...
import {
  ...
  User, ClipboardList, LogOut, ArrowRight,
  Wind, ShoppingBag, RefreshCw,
} from "lucide-react";
...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 bg-white/95 backdrop-blur-md transition-all duration-300 ${scrolled ? "shadow-lg shadow-black/5" : "shadow-sm"}`}>
      <div className="w-full mx-auto px-[40px] h-[72px] flex items-center justify-between">
          ...
          <div className="relative" ref={accountRef}>
            {user ? (
              <>
                ...
                {accountOpen && (
                  <div className="absolute right-0 top-full mt-3 w-52 bg-white border border-gray-100 rounded-2xl shadow-2xl shadow-black/10 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <Link href="/my-account" onClick={() => setAccountOpen(false)}
                      className="flex items-center gap-2.5 px-4 py-3 text-sm text-[#3d2b1f] hover:bg-[#f5f0eb] transition-colors font-medium">
                      <User className="w-4 h-4 text-gray-400" />
                      My Account
                    </Link>
                    <Link href="/my-account?tab=orders" onClick={() => setAccountOpen(false)}
                      className="flex items-center gap-2.5 px-4 py-3 text-sm text-[#3d2b1f] hover:bg-[#f5f0eb] transition-colors font-medium border-t border-gray-50">
      ...
      {shopOpen && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 w-full bg-white border-t-2 border-[#5a8c3a] shadow-2xl z-40">
            <div className="w-full mx-auto px-[40px] py-6">
                ...
                {CATEGORIES.map((cat) => (
                  <Link key={cat.href} href={cat.href} onClick={() => setShopOpen(false)}
                    ...
                    {cat.image ? (
                      ...
                      <div className="absolute inset-0 flex items-center justify-center" style={{ background: cat.gradient }}>
                        {cat.icon}
                      </div>
                    )}
      ...
      {searchOpen && (
        <>
          ...
          <div className="absolute top-full left-0 right-0 bg-white border-t border-gray-100 shadow-xl z-40">
            <div className="w-full mx-auto px-[40px] py-5">
