...
export default function ProductGallery({ images, name }: { images: Img[]; name: string }) {
  ...
  return (
    <div className="flex flex-col gap-3 lg:self-stretch h-full">
      {/* Main image — fills available height, matching the info card */}
      <div className="relative flex-1 min-h-[380px] bg-[#f8f6f3] rounded-2xl overflow-hidden border border-gray-100">
        <Image src={main.src} alt={main.alt || name} fill
          className="object-contain p-4" sizes="(max-width:768px) 100vw, 50vw" priority />
      </div>

      {/* Thumbnail strip — always shown below, even for a single image */}
