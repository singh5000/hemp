    ...
    <div className="space-y-5">
              ...
              {attr.terms.map(term => {
                ...
                return (
                  <button key={term.id}
                    ...
                    className={`px-4 py-2 text-[14px] font-semibold rounded-lg border-2 transition-all ${
                      isActive
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
          ...
          <span className="w-10 text-center text-[16px] font-bold text-[#3d2b1f]">{qty}</span>
        ...
        <button onClick={handleAdd} disabled={!canAddToCart || cartState === "loading"}
          className={`flex-1 py-3 text-[16px] font-bold uppercase tracking-wider rounded-xl transition-all duration-200 ${
            !canAddToCart
            ...
            <>
              {notifyState === "sent" ? (
                <div className="py-3 px-4 rounded-xl bg-[#1A9248]/10 border border-[#1A9248]/20 flex items-center gap-2">
                  ...
                  <span className="text-[16px] font-bold text-[#1A9248]">We&apos;ll notify you when this product is back in stock!</span>
                </div>
              ) : notifyOpen ? (
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        required
                        className="w-full pl-10 pr-3 py-3 text-[16px] border border-gray-200 rounded-xl focus:outline-none focus:border-[#1A9248] text-[#3d2b1f] placeholder:text-gray-300"
                      />
                    </div>
                    <button type="submit" disabled={notifyState === "sending"}
                      ...
                      {notifyState === "sending" ? (
                        ...
                        <>
                          ...
                          <span className="text-[16px] font-bold">Notify Me</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                <button onClick={() => setNotifyOpen(true)}
                  className="w-full py-3 text-[16px] font-bold uppercase tracking-wider rounded-xl border-2 border-dashed border-gray-200 hover:border-[#1A9248] hover:bg-[#1A9248]/5 text-gray-500 hover:text-[#1A9248] transition-all duration-300 flex items-center justify-center gap-2 group">
